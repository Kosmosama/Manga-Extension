...
- [ ] Establish naming convention for keys (e.g., `manga.card.favorite`, `settings.theme.label`, `tags.list.empty`).
- [ ] Create/enrich base language files:
  - [ ] en.json (complete baseline, migrate keys from legacy `translations.js`).
  - [ ] es.json (synchronized with English).
- [ ] Implement a language selector component/page (integrate with `SettingsService.changeLanguage()`).
...
- [ ] Missing key logging:
  - [ ] Disable `logMissingKey` in production builds.
...
### System mode support
- [ ] Extend Settings UI to include “System” option alongside Light/Dark (Theme enum already has `System`).
- [ ] Detect system preference via `prefers-color-scheme` (already in `ThemeService.getSystemTheme()`).
- [ ] Listen for OS theme changes when System mode is active (`matchMedia('(prefers-color-scheme: dark)').addEventListener('change', ...)`).
- [ ] Unify approach: choose between using `data-theme="dark|light"` vs toggling a `dark` class. Ensure consistency across CSS.
- [ ] Persist user choice (`chrome.storage.local` already used). Add migration if stored values were only light/dark in older versions.
...
### Resilience
- [ ] Early apply theme script BEFORE Angular bootstrap (avoid flash):
  - [ ] Inline snippet to read stored theme and set `data-theme` or `class="dark"` if needed.
  - [ ] Fallback to system preference if storage not yet available.
- [ ] Sync across multiple open extension views via `chrome.storage.onChanged`.
...
### Service lifecycle
- [ ] Fix `SettingsService`: move initialization logic (currently in `ngOnInit`) into constructor or an explicit `init()` called from `provideAppInitializer`.
- [ ] Add defensive checks around storage access (graceful fallback if Chrome APIs unavailable—e.g., testing environment).

### Manga card chapter updates
- [ ] Replace current `map()` producing Observables with `switchMap/concatMap`.
- [ ] Implement batching: accumulate increments for a short debounce period before persisting.
- [ ] Provide busy state (while persisting).

### Fallback images
- [ ] Move assets to `src/assets/fallback-images/`.
- [ ] Reference via `assets/...` paths (Angular’s asset handling).
- [ ] Add actual `<img (error)="imageNotFound()">` usage.
- [ ] Provide generic icon fallback if both themed images fail.
...
- [ ] Focus search input toggle (already stored, needs UI).
- [ ] Real-time sync via storage change events.
...
- [ ] Chapter update batching (see Code Quality).
